...
class GreenValleySystem:
    ...
    def create_walkin_group(self, phones: list[str]):
        created_guests = []
        current_guest_count = len([u for u in self.__users if isinstance(u, Guest)])
        
        for index, phone in enumerate(phones):
            running_number = current_guest_count + index + 1
            new_id = f"G-{running_number:02d}"
            
            new_guest = Guest(user_id=new_id, name=f"Walk-in {new_id}", phone=phone)
            self.__users.append(new_guest)
    ...
    def issue_raincheck_to_user(self, user_id: str, amount: float): #✔️
        ...
        if user and isinstance(user, Golfer):
            ...
            self.create_notification(user, msg)
...
# ------------------- booking ----------------------- #
    ...
    def issue_raincheck_to_user(self, user_id: str, amount: float):
        ...
        if user and isinstance(user, Golfer):
            # สร้าง Code และดึงเบอร์โทรมาสร้าง Object
            auto_code = f"RC-{len(self.__rain_checks) + 1:04d}-{user_id}"
            new_rc = Raincheck(code=auto_code, amount=amount, phone=user.phone)
            # เพิ่มเข้า system และ user
            self.__rain_checks.append(new_rc)
            user.add_raincheck(new_rc)
...
# -------------- notification ------------------ #
    ...
    # เช็คว่าเป็น member หรือ guest ค่อยเพิ่ม noti ให้ 
    def create_notification(self, user, message: str):
