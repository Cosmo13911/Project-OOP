...
def walk_in_booking(
    ...
    all_phones_in_group = [phone] + companion_phones
    ...
    # สร้าง guest_id (W-เบอร์โทร 4 ตัวท้าย-ลำดับ guest ในระบบ)
    current_guest_count = len([u for u in sys.users if isinstance(u, Guest)])
    lead_no = current_guest_count + 1
    lead_guest_id = f"W-{phone[-4:]}-{lead_no}"
    lead_guest = Guest(lead_guest_id, guest_name, phone)
    sys.users.append(lead_guest)
      
    slot.status = SlotStatus.RESERVED
    b_id = f"WK-{len(sys.bookings) + 1:03d}"
    new_booking = Booking(b_id, lead_guest, slot)
    
    # 2. สร้างลูกก๊วนจากเบอร์โทรที่กด Add เพิ่มเข้ามา + ใส่ชื่ออัตโนมัติ
    for i, comp_phone in enumerate(companion_phones, start=1):
        validate_phone(comp_phone)
        
        comp_no = lead_no + i 
        
        comp_id = f"W-{comp_phone[-4:]}-{comp_no}"
        comp_name = f"เพื่อนของ {guest_name} ({i})"
        
        comp_guest = Guest(comp_id, comp_name, comp_phone)
        sys.users.append(comp_guest)
